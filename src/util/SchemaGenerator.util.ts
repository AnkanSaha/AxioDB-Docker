...
export default function generateSchema(input: SchemaInput): Record<string, any> {
    ...
    for (const [key, value] of Object.entries(input)) {
        ...
        for (const mod of modifiers) {
            ...
            else if (mod.startsWith("min:")) field = field.min(Number(mod.split(":")[1]));
            else if (mod.startsWith("max:")) field = field.max(Number(mod.split(":")[1]));
            else if (mod.startsWith("length:")) field = field.length(Number(mod.split(":")[1]));
